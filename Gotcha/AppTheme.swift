//
//  AppTheme.swift
//  Gotcha
//
//  Created by Rajahiresh Kalva on 1/28/26.
//

import SwiftUI

struct AppTheme {
    // MARK: - Logo Colors
    // The bright neon green from the lightning bolt/accents
    static let brandGreenLight = Color(hex: "A4EB14")
    
    // The deeper green from the outer rings/gradient
    static let brandGreenDark = Color(hex: "5E8C22")
    
    // Dark text color for readability against white cards
    static let deepText = Color(hex: "1C1C1E")
    
    // MARK: - Main Gradient Background
    // This replicates the green background of your logo icon.
    // Use this as: AppTheme.mainBackground.ignoresSafeArea()
    static let mainBackground = LinearGradient(
        gradient: Gradient(colors: [brandGreenLight, brandGreenDark]),
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )
}

// MARK: - View Extensions
extension View {
    /// Applies the rounded design font to match the logo's aesthetic.
    /// Usage: .brandFont(size: 18, weight: .bold)
    func brandFont(size: CGFloat? = nil, weight: Font.Weight = .regular) -> some View {
        self.font(.system(size: size ?? 17, weight: weight, design: .rounded))
    }
}

// MARK: - Color Hex Extension
extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (1, 1, 1, 0)
        }
        self.init(.sRGB, red: Double(r) / 255, green: Double(g) / 255, blue: Double(b) / 255, opacity: Double(a) / 255)
    }
}
